...
object PlotTest2 extends App {
  ...
  val errors = Source.fromFile("/home/nkatz/Desktop/test-1").getLines.filter( x => !x.isEmpty && !x.startsWith("%"))//.split(",")
  ...
  val winnow_refs = errors.next().split(",").map(_.toDouble).toVector
  val winnow_09 = errors.next().split(",").map(_.toDouble).toVector
  ...
  Figure("sin_vs_cosine").plot(_09).plot(_08).plot(_07).plot(winnow).plot(winnow_refs).plot(winnow_09)//.plot(_09New)//.plot(winnow)
    //.plot(domain -> sin _)
    //.plot(lineStyle = DASHED)(domain -> cos _)
    .havingLegends("0.9", "$0.8$", "0.7", "winnow", "winnow-refs", "winnow_09")//, "$0.75$", "$0.95$", "winnow") //"no constraints")

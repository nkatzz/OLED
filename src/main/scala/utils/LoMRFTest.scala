...
//  )
  ...
  val (kb, constants) = KB.fromFile("/home/nkatz/dev/move_HI.mln")
  ...
  val rules = List(
    "1 InitiatedAt(move(id1, id2), t) :- Happens(walking(id1), t)",
    "1 TerminatedAt(move(id1, id2), t) :- Happens(active(id1), t)"
  )
  ...
  //Evidence.fromFiles(kb,constants,queryPredicates,Vector(new File("some file with the data")),false,true)
  /*------------------------------------------------------------------------------------------------------*/

  val constantsDomain = Map(
    "time" -> ConstantsSet((0 to 1).map(_.toString)),
    "fluent" -> ConstantsSet("Move_A_B"),
  ...
  builder.evidence += EvidenceAtom.asTrue("Happens", Vector[Constant](Constant("Walking_A"), Constant(0.toString)))
  ...
  builder.functions += FunctionMapping("Move_A_B", "move", Vector(Constant("A"), Constant("B")))
  ...
  annotationBuilder.evidence +=
    EvidenceAtom.asTrue("InitiatedAt", Vector[Constant](Constant("Move_A_B"), Constant(0.toString)))
  annotationBuilder.evidence +=
    EvidenceAtom.asFalse("InitiatedAt", Vector[Constant](Constant("Move_A_B"), Constant(1.toString)))

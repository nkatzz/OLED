...
import all.structures.Theory
import all.utils.ASP
...
object Rules extends LazyLogging{
  ...
  def getRefinedProgram(incRule: InconstistentRule,
                        ...
                        newRules: Theory, examples: Example, jep: Jep): InconstistentRule = {
    ...
    def search: InconstistentRule = {
      ...
      for (ssc <- incRule.rule.supportSet.clauses){
        ...
        val (_,_,_,_,defeasibleRule,use3Map) =
          ASP.inductionASPProgram( retained = retainedRules.extendUnique(newRules),
            findAllRefs = (incRule.rule,ssc), examples = examples.toMapASP, aspInputFile = file)
        val answerSets = ASP.solve(Core.FIND_ALL_REFMS, use3Map, file, examples.toMapASP, jep=jep)
      ...
      Core.glvalues("refinementSearch") match {
    ...
    def tryToRefine(priorTheory: Theory, retained: Theory, examples: Example, jep: Jep): List[AnswerSet] = {
      val file = Utils.getTempFile("search",".lp")
      val (_,_,defeasiblePrior, use3Map,_,_) =
        ASP.inductionASPProgram(priorTheory = priorTheory, retained = retained,
          examples = examples.toMapASP, aspInputFile = file)
      val answerSets = ASP.solve(Core.FIND_ALL_REFMS, use3Map, file, examples.toMapASP, jep=jep)
    ...
    val refinement: InconstistentRule =
      ...
      if (retainedRules.extend(newRules).extend(Theory(incRule.initialRefinement)).thetaSubsumes(incRule.rule.supportSet)){
        ...
        val answerSets = tryToRefine(priorTheory = Theory(incRule.rule), retained = retainedRules.extendUnique(newRules), examples = examples, jep=jep)
        //InconstistentRule(incRule.rule, Theory(refinement))
  ...
  def refineRule(incRule: InconstistentRule, retained: Theory,
                 e: Example, withSupport:String = "fullSupport", jep: Jep): Either[String,List[InconstistentRule]] = {
    val file = Utils.getTempFile("search",".lp")
    val (_,_,_,use3Map,_,_) = ASP.inductionASPProgram(
      priorTheory = Theory(incRule.rule), retained = retained,
      examples = e.toMapASP, aspInputFile = file, use3WithWholeSupport = true, withSupport = withSupport)
    val answerSets = ASP.solve(Core.FIND_ALL_REFMS, use3Map, file, e.toMapASP, jep=jep)
    val refined =
    answerSets match {
      case Nil =>
  ...
  def getRefined(incs: List[InconstistentRule], retainedRules: Theory,
                 newRules: Theory, e: Example, withSupport:String = "fullSupport", jep: Jep) = {
    ...
    for (incRule <- incs) {
      if (searchMore(incRule,Theory(incRule.initialRefinement))){
        val r = refineRule(incRule,retained,e,withSupport=withSupport, jep=jep)
        val z = r match {
          case Left(x) =>
            logger.error(x)
            if (Core.glvalues("perfect-fit") == "true") {

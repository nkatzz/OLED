...
object ILEDHoeffdingNoSupportTermInit extends GlobalValues with LazyLogging {
  ...
  def getData = all.utils.CaviarUtils.getDataAsChunks(DB, chunkSize, initorterm, withInertia).take(trainigsize)
  ...
  def runWithDataChunks(DB: Database, jep: Jep, targetClass: String, withInertia: Boolean): Theory = {
    ...
    def runOnce(inTheory: Theory): Theory = {
      ...
      val data = all.utils.CaviarUtils.getDataAsChunks(DB, chunkSize, initorterm, withInertia).take(trainigsize)
      //val data = getData
...
object DevUtils {
  ...
  def accumulateKernels(DB: Database, jep: Jep) = {
    ...
    while (dataIterator.hasNext) {
      ...
      val tt = all.utils.Utils.generateKernel(e.exmplNoInertia.toMapASP,fromWeakExmpl = false, jep)
      val varKernel = tt._2

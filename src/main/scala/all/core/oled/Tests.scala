...
import all.utils.{ASP, CaviarUtils, Database}
import jep.Jep
...
*/
  ...
  */
  ...
  def jepMemoryLeakTests = {
    // Tests for Jep memory leak
    val jep = new Jep()
    var i = 0
    while (true) {
      println(i)
      ASP.solve(task = GlobalValues.SCORE_RULES, aspInputFile = new File("/home/nkatz/Desktop/test"), jep=jep)
      i += 1
    }
  }

  //CaviarUtils.mergeCaviarCopies(2)

...
import all.utils.Utils.{lined, time}
import all.utils.{ASP, Database, MongoUtils, Utils}
...
object Iled extends ClausalLogicParser with LazyLogging with MongoUtils {
  ...
  def revise(kernelSet: Theory = Theory(),
    ...
    val answerSet = ASP.solve("all", use2AtomsMap ++ use3AtomsMap, aspFile, example.toMapASP, fromWeakExmpl = fromWeakExmpl, jep)
    if (answerSet != Nil) {

...
import all.core.oled.old_stuff.ILEDHoeffdingNoSupportTermInit
...
import all.structures.{Constant, Literal}
import jep.Jep
...
object ParseCAVIAR extends ClausalLogicParser{
  ...
  def countInterpretations(path: String) = {
    ...
    for (f <- innerFolders) {
      ...
      val size = if (ids > 1) (times * all.utils.Utils.combinations(ids,2)).toInt else times
...
object CRIMEAData extends App{
  ...
  def toFile(lines: List[Interpretation]) = {
    ...
    all.utils.Utils.writeLine(y,"/home/nkatz/Desktop/kernel.txt","overwrite")
  }

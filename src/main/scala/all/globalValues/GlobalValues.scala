 ...
 */
  ...
  var fromDB = "CAVIAR_Real_FixedBorders"
  //val fromDB = "CAVIAR_meeting_CLEAN"
  //val fromDB = "CAVIAR_moving_CLEAN"
  //val fromDB = "CAVIAR-MERGED-COPIES-10"
  //val fromDB = "CAVIAR-MERGED-COPIES-8"
  //val fromDB = "CAVIAR-MERGED-COPIES-5"
  //val fromDB = "CAVIAR-MERGED-COPIES-2"
  ...
  //val inputPath = s"$cwd/datasets/RevealTwitterData"
  var inputPath = s"$cwd/datasets/Caviar"

  def modesFile = inputPath + "/modes"     // Mode Declarations file

  val ASPHandler = s"$cwd/asp/ASPHandler.py"

  def BK_INITIATED_ONLY = inputPath + "/bk-initiated-only.lp"
  def BK_TERMINATED_ONLY = inputPath + "/bk-terminated-only.lp"
  def BK_INITIATED_ONLY_MARKDED = inputPath + "/bk-score-initiated.lp"
  def BK_TERMINATED_ONLY_MARKDED = inputPath + "/bk-score-terminated.lp"
  def BK_WHOLE_EC = inputPath + "/bk.lp"
  def BK_CROSSVAL = inputPath + "/bk-for-crossval.lp"
  def BK_OLED_WITH_INERTIA = inputPath + "/bk-oled-with-inertia.lp"

  def HAND_CRAFTED_RULES = new java.io.File(inputPath).getParentFile.getCanonicalPath + "/hand-crafted-rules/caviar"
}

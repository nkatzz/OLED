...
class Learner[T <: app.runutils.IOHandling.Source](inps: RunningOptions, trainingDataOptions: T,
  ...
  override def processingState: Receive = {
    ...
    case batch: Example =>
      ...
      var inferredState = WoledUtils.getInferredState(Theory.compressTheory(rules), e, "MAP", inps)
      ...
      val (tpCounts, fpCounts, fnCounts, totalGroundings) = Scoring.scoreAndUpdateWeights(e, inferredState, state.getAllRules(inps.globals, "all").toVector, inps, logger)

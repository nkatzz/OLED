...
import iled.structures.{Clause, Theory}
import iled.utils.{CaviarUtils, Utils, Exmpl, Database}
import jep.Jep
...
class TheoryLearner(val DB: Database, val delta: Double, val breakTiesThreshold: Double,
  ...
  def getData = iled.utils.CaviarUtils.getDataAsChunks(DB, chunkSize, targetClass, withInertia).take(trainingSetSize)
  ...
  def run: (Theory,Double) = {
    def runOnce(inTheory: Theory): Theory = {
      ...
      data.foldLeft(inTheory){ (topTheory, newExample) =>
        ...
        //println(newExample.exmplWithInertia.narrative)
        val res = Utils.time {
          processExample(topTheory, newExample, learningInitWithInertia)
        }
        println(s"${res._2} sec")
        res._1
      }
    }

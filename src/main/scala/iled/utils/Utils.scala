...
object Utils extends ASPResultsParser with LazyLogging{
  ...
  */
  ...
  def checkIfCompressed(theory: PriorTheory) = {
    val compressed = Xhail.compressTheory(theory.merge.clauses)
  ...
  def generateKernel(examples: Map[String,List[String]], fromWeakExmpl: Boolean = false,
                     jep: Jep, bk: String = Core.bkFile, learningTerminatedOnly: Boolean=false, oledLearningInitWithInertia: Boolean=false) = {
    ...
    var (kernel, varKernel) =
      Xhail.runXhail(fromFile = infile.getAbsolutePath, kernelSetOnly = true,
        fromWeakExmpl = fromWeakExmpl, jep=jep, bkFile=bk, learningTerminatedAtOnly=learningTerminatedOnly, oledLearningInitWithInertia=oledLearningInitWithInertia)
    if (fromWeakExmpl) {
